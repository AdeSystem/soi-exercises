...
PRIVATE message m;
PRIVATE int last_user_priority = 50;
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  if (last_user_priority == 50) {
    rpc->user_priority = 10;
    last_user_priority = 10;
  } else {
    rpc->user_priority = 50;
    last_user_priority = 50;
  }
...
PRIVATE int do_getpri(message *m_ptr) {
