...
int count_children( int proc_nr ) {
  /* Counting given process's children */
  int children = 0;
  int i = 0;
    /* Iterating through processes in order to count children */
...
void most_children( int * children, pid_t * who ) {
  int most_children_ammount = -1;
  pid_t found = -1;
  int count = 0;
  int proc_nr = 0;
  /* Iterating through processes in order to find one with the most children */
  for (proc_nr = 0; proc_nr < NR_PROCS; ++proc_nr) {
    if ((mproc[proc_nr].mp_flags & IN_USE) && (mproc[proc_nr].mp_pid != 0)) {
      count = count_children( proc_nr );
      if (count > most_children_ammount) {
        most_children_ammount = count;
        found = mproc[proc_nr].mp_pid;
      }
    }
  }
  *children = most_children_ammount;
  *who = found;
}

PUBLIC int do_who_most_children( void ) {
  /* MOST CHILDREN SYSCALL */
  register struct mproc *result = mp;
  int children = -1;
  pid_t who = -1;
  most_children( & children, & who );

  result->reply_res2 = who;

  return children;
}

int count_descendants(int proc_nr) {
  int descendants = 0;
  int i = 0;
  int stack[NR_PROCS];
  int pointer = -1;
  int current_proc;
  int visited[NR_PROCS] = {0};

  /* Pushing given process to stack and marking it as visited in order to prevent infinity loop */
  stack[++pointer] = proc_nr;
  visited[proc_nr] = 1; 

  while (pointer >= 0) {

    /* Poping process from stack */
    current_proc = stack[pointer--];

    /* Iterating through processes in order to find descendats of current process */
    for (i = 0; i < NR_PROCS; ++i) {
      if ((mproc[i].mp_flags & IN_USE) && mproc[i].mp_parent == current_proc && !visited[i]) {
        descendants++;
        /* Pushing current process's descentand to stack in order to check them in future iterations */
        stack[++pointer] = i;
        /* Marking them as visited in order to prevent infinity loop */
        visited[i] = 1;
      }
    }
  }
...
int most_descendants( int * descendants, pid_t * who, pid_t exclude_pid) {
  int most_descendants_ammount = -1;
  pid_t found = -1;
  int count = 0;
  int proc_nr = 0;
  /* Iterating through processes in order to find one with the most descendants */
  for (proc_nr = 0; proc_nr < NR_PROCS; ++proc_nr) {
    if ((mproc[proc_nr].mp_flags & IN_USE) && (mproc[proc_nr].mp_pid != exclude_pid) && (mproc[proc_nr].mp_pid != 0)) {
      count = count_descendants(proc_nr);
      if (count > most_descendants_ammount) {
        most_descendants_ammount = count;
        found = mproc[proc_nr].mp_pid;
      }
    }
  }
  *descendants = most_descendants_ammount;
  *who = found;
}

PUBLIC int do_who_most_descendants( void ) {
  /* MOST DESCENDANTS SYSCALL */
  register struct mproc *result = mp;
  int descendants = -1;
  pid_t who = -1;
  pid_t exclude_pid = mm_in.m1_i1;
  most_descendants( &descendants, &who, exclude_pid);
