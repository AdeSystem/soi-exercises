...
PRIVATE int current_algorithm = FIRST; 
 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *biggest, *biggest_prev;
  phys_clicks old_base;

  if(current_algorithm == WORST)
  {
	do {
		biggest_prev = NIL_HOLE;  
		biggest = hp = hole_head; 
		/* Find the biggest possible hole */
		while (hp != NIL_HOLE && hp->h_base < swap_base) 
		{
			if(hp->h_len > biggest->h_len)
			{
				biggest = hp;
		...
		/* Using the biggest hole found in previous step */
		if (biggest->h_len >= clicks) 
		{
			old_base = biggest->h_base;	
			biggest->h_base += clicks;	
			biggest->h_len -= clicks;	

			if (biggest->h_len == 0) del_slot(biggest_prev, biggest);
...
{
	register struct hole *it;  
	
	phys_clicks buffer[NR_HOLES * 2 + 1], size, i, to_copy;
	
	/* Max Buffer size */
	size = (mm_in.m1_i1 - sizeof(phys_clicks)) / (sizeof(phys_clicks)); 
	/* Current buffer size (empty) */
	i = 0; 

	for(it = hole_head; it != NIL_HOLE && it->h_base < swap_base && i < size-1; it = it->h_next) 
	{
		buffer[i++] = it->h_len;  
		buffer[i++] = it->h_base;  
	}
	...
	to_copy = (phys_clicks) (i * sizeof(phys_clicks)); 

	sys_copy(MM_PROC_NR, D, (phys_clicks) buffer, who, D, (phys_clicks) mm_in.m1_p1, to_copy); 
...
{
  int algorithm = mm_in.m1_i1;

  if(algorithm == FIRST)
  {
	current_algorithm = FIRST; 
  }
  else if(algorithm == WORST)
  {
	current_algorithm = WORST; 
  }
  else {
	return -1;
  }
